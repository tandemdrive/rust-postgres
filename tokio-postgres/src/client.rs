...
use crate::error::RowCountCategory;
...
impl InnerClient {
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self, messages)))]
...
impl Client {
    ...
    /// An uninstrumented version of `query_one`.
        ...
        T: ?Sized + ToStatement + fmt::Debug,
    {
        self.query_opt_intern(statement, params)
            .await
            .and_then(|res| {
                res.ok_or_else(|| Error::row_count(RowCountCategory::One, RowCountCategory::Zero))
            })
    }
    ...
    /// An uninstrumented version of `query_opt`.
    async fn query_opt_intern<T>(
        &self,
        statement: &T,
        params: &[&(dyn ToSql + Sync)],
    ) -> Result<Option<Row>, Error>
        ...
        while let Some(row) = stream.try_next().await? {
            if first.is_some() {
                return Err(Error::row_count(
                    RowCountCategory::ZeroOrOne,
                    RowCountCategory::MoreThanOne,
                ));
            }
